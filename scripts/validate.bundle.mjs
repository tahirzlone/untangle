...
// scripts/validate-pure.mjs
var import__ = __toESM(require__(), 1);
var import_ajv_formats = __toESM(require_dist(), 1);
function createValidator(schema2) {
  const ajv = new import__.default({ allErrors: true });
  (0, import_ajv_formats.default)(ajv);
  const validateSchema = ajv.compile(schema2);
  return function validateWorkflow2(doc2) {
    const errors2 = [];
    if (!validateSchema(doc2)) {
      for (const e of validateSchema.errors ?? []) {
        errors2.push(`schema: ${e.instancePath || "/"} ${e.message}`);
      }
      return { valid: false, errors: errors2 };
    }
    const nodeIds = new Set(doc2.nodes.map((n) => n.id));
    if (nodeIds.size !== doc2.nodes.length) {
      errors2.push("integrity: duplicate node ids");
    }
    doc2.edges.forEach((e, i) => {
      if (!nodeIds.has(e.from)) errors2.push(`integrity: edges[${i}].from "${e.from}" is not a node id`);
      if (!nodeIds.has(e.to)) errors2.push(`integrity: edges[${i}].to "${e.to}" is not a node id`);
    });
    doc2.suggestions.forEach((s, i) => {
      if (!nodeIds.has(s.nodeId)) {
        errors2.push(`integrity: suggestions[${i}].nodeId "${s.nodeId}" is not a node id`);
      }
      const allowed = new Set(nodeIds);
      if (s.effect.replaceWith) allowed.add(s.effect.replaceWith.id);
      for (const id of s.effect.removeNodes) {
        if (!nodeIds.has(id)) errors2.push(`integrity: suggestions[${i}].effect.removeNodes "${id}" is not a node id`);
      }
      for (const id of s.effect.mergeNodes) {
        if (!nodeIds.has(id)) errors2.push(`integrity: suggestions[${i}].effect.mergeNodes "${id}" is not a node id`);
      }
      s.effect.newEdges.forEach((e, j) => {
        if (!allowed.has(e.from)) errors2.push(`integrity: suggestions[${i}].effect.newEdges[${j}].from "${e.from}" is unknown`);
        if (!allowed.has(e.to)) errors2.push(`integrity: suggestions[${i}].effect.newEdges[${j}].to "${e.to}" is unknown`);
      });
    });
...
var validateWorkflow = createValidator(schema);
...
var { valid, errors } = validateWorkflow(doc);
if (!valid) {

...
it('states the graph in the toolbar: title, counts, pain, KB', async () => {
  render(<GraphCanvas workflow={wf} />);
  await waitFor(() => {
    expect(screen.getAllByTestId('sg-node')).toHaveLength(wf.nodes.length);
  });
  ...
  // The generator id stays in the file as provenance and stays off the screen:
  // a graph drawn by any model reads the same, so naming one in the chrome only
  // ever spoke for the run that happened to make it.
  expect(toolbar).not.toHaveTextContent(wf.meta.model);
});

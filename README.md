...
- **Press APPLY on a card** and watch the graph simplify: the steps it replaces collapse into one, the ember cools, the impact meter counts up what you got back (steps, minutes, tokens, manual interventions), and a new version lands on the version strip.
- **Press OPTIMIZE** to hand the whole graph over: the camera travels to each step worth upgrading, applies it, and moves on while the flow collapses behind it. It ends on a scorecard — how many upgrades landed, the four totals, before→after node count, and what was applied. CANCEL or ESC stops the run after the patch in flight.
...
- **Hold VS ORIGINAL** to see the graph you started with, ghosted under the graph you have now; let go and it is gone. It holds through pan and zoom, so the two line up card for card.
